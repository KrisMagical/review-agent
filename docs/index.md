# Documentation Index

Start here if you are evaluating, installing, deploying, or extending MagicReview.

The current Python package is `magicreview`, and the primary CLI command is `mgreview`.

## User Guides

- [CLI](cli.md)
- [Docker](docker.md)
- [Deployment](deployment.md)
- [Self Hosting](self_hosting.md)
- [Auth](auth.md)
- [Dashboard](dashboard.md)
- [Hosted Review Web UI](hosted_review.md)
- [Public Beta](public_beta.md)

## Integrations

- [GitHub App](github_app.md)
- [MCP](mcp.md)
- [Model Providers](model_providers.md)
- [Connected Services](connected_services.md)

## Review Capabilities

- [Enterprise Rules](enterprise_rules.md)
- [Architecture Review](architecture_review.md)
- [Multi-Agent Review](multi_agent.md)

## Operations

- [Privacy](privacy.md)
- [Release](release.md)

## Recommended Reading Paths

Local CLI:

1. [CLI](cli.md)
2. [Privacy](privacy.md)
3. [Enterprise Rules](enterprise_rules.md)

Self-hosted Dashboard:

1. [Docker](docker.md)
2. [Auth](auth.md)
3. [Self Hosting](self_hosting.md)
4. [Dashboard](dashboard.md)

GitHub PR review:

1. [GitHub App](github_app.md)
2. [Connected Services](connected_services.md)
3. [Privacy](privacy.md)

LLM review:

1. [Model Providers](model_providers.md)
2. [Connected Services](connected_services.md)
3. [Privacy](privacy.md)



