"""FastAPI Dashboard for MagicReview governance data."""
...
from magicreview.dashboard.hosted_review import HostedReviewService, max_upload_bytes
from magicreview.dashboard.model_settings import ModelProviderTester, ModelSettingsRepository
...
app = FastAPI(title="MagicReview Dashboard")
...
def dashboard_host_port() -> tuple[str, int]:
    return (
        get_env("DASHBOARD_HOST", "127.0.0.1") or "127.0.0.1",
        int(get_env("DASHBOARD_PORT", "8080") or "8080"),
    )
...
def dashboard_index(request: Request) -> Any:
    if request is None:
        return "<h1>MagicReview Dashboard</h1>"
    if templates is None:
        return HTMLResponse("<h1>MagicReview Dashboard</h1>")
...
def main() -> None:
    ...
    uvicorn.run("magicreview.dashboard.app:app", host=host, port=port)
