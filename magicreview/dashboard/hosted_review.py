...
from magicreview.dashboard.model_settings import ModelSettingsRepository
...
def max_upload_bytes() -> int:
    try:
        return int(get_env("MAX_UPLOAD_BYTES", "5242880") or "5242880")
    except ValueError:
        return 5 * 1024 * 1024


def allowed_review_roots() -> list[Path]:
    configured = get_env("ALLOWED_REVIEW_ROOTS", "") or ""
...
def ensure_allowed_path(path: str, *, roots: list[Path] | None = None) -> Path:
    ...
    if not any(_is_relative_to(resolved, root) for root in allowed):
        raise ValueError("Path is outside MGREVIEW_ALLOWED_REVIEW_ROOTS.")
    return resolved
