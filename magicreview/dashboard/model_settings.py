...
ENV_KEY_BY_PROVIDER = {
    ...
    "enterprise_gateway": "ENTERPRISE_LLM_API_KEY",
}
...
class ModelProviderSettings:
    ...
    def effective_api_key(self) -> str:
        ...
        env_value = _provider_env(env_name)
        if env_value:
            return env_value
...
class ModelProviderTester:
    ...
    def _test_network_provider(self, settings: ModelProviderSettings) -> dict[str, Any]:
        ...
        if settings.provider == "ollama":
            base_url = settings.base_url or get_env("OLLAMA_BASE_URL") or "http://localhost:11434"
...
def settings_from_env(settings: ModelProviderSettings) -> ModelProviderSettings:
    ...
    settings.model = get_env("LLM_MODEL") or settings.model
    settings.base_url = (
        get_env("OPENAI_COMPATIBLE_BASE_URL")
        or get_env("OLLAMA_BASE_URL")
        or get_env("ENTERPRISE_LLM_BASE_URL")
...
def _completion_url(settings: ModelProviderSettings) -> str:
    ...
    if settings.provider == "enterprise_gateway":
        return f"{(settings.base_url or get_env('ENTERPRISE_LLM_BASE_URL', '') or '').rstrip('/')}/chat/completions"
    return f"{settings.base_url.rstrip('/')}/chat/completions"


def _provider_env(env_name: str | None) -> str:
    if not env_name:
        return ""
    if env_name.startswith(("OPENAI_", "ANTHROPIC_", "AZURE_")):
